<template>
  <section
    class="px-24 py-4 flex -mt-10 mb-10 bg-green-600 text-center"
    id="sponzori"
  >
    <div class="flex justify-between items-center w-8/12 mx-auto py-10 text-purple-100">
      <h2 class="text-4xl font-bold">Tak co, už se k nám přidáš?</h2>
      <a
        href="//m.me/klubinvestoru"
        target="_blank"
      >
        <button
          class="rounded-full text-green-700 bg-white shadow-md py-5 px-8 text-xl uppercase tracking-wider font-bold flex items-center mx-auto hover:shadow-lg trans"
        >
          <span>Jdu do toho!</span></button>
      </a>
    </div>
  </section>
</template>
