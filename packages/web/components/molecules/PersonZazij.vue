<template>
  <div class="flex items-center mt-10 w-1/2 px-8">
    <VImage class="h-48 w-48 flex-shrink-0" :publicId="photo" :alt="alt" />
    <div class="bg-white py-6 px-6 shadow-md rounded-r flex-grow">
      <div class="flex justify-between">
        <h4 class="text-lg font-bold">
          <slot name="name">Martin</slot>
        </h4>
        <unicon name="linkedin" class="flex-shrink-0" />
      </div>
      <h5
      ...
      <div class="flex justify-between items-end">
        <q class="text-lg"> <slot name="quote"/></q>
      </div>
    </div>
  </div>
</template>
...
export default {
  props: {
    photo: {
      type: String,
      default: "nabor/martin"
    },
    alt: {
      type: String,
      default: "Předseda Klubu investorů Martin Hrdý"
    }
  }
};
</script>
